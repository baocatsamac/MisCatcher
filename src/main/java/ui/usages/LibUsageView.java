...
import ui.dependency.DependencyDoubleClickHandler;
...
public class LibUsageView implements UsageView {
    @NonNls
...
//    private final ExporterToTextFile myTextFileExporter = new ExporterToTextFile(this);
    ...
    private java.util.List<UsageContextPanel.Provider> myUsageContextPanelProviders;
    ...
    @Nullable
    private Runnable myRerunActivity;

    public LibUsageView(@NotNull final Project project,
                ...
                synchronized (lock) {
                    ...
                    myTree = new Tree(myModel) {
                        ...
                        public Enumeration<TreePath> getExpandedDescendants(TreePath parent) {
                            return expandingAll ? Collections.emptyEnumeration() : super.getExpandedDescendants(parent);
                        }
                    };
        ...
        myExclusionHandler = new ExclusionHandler<DefaultMutableTreeNode>() {
            ...
            private void collectParentNodes(DefaultMutableTreeNode node, Set<Node> nodes, boolean almostAllChildrenExcluded) {
                ...
                java.util.List<Node> otherNodes =
    ...
    private void fireEvents() {
        ...
        java.util.List<Node> insertedUnder;
        synchronized (nodesInsertedUnder) {
        ...
        java.util.List<Node> nodesToFire = new ArrayList<>();
        for (Node parentNode : insertedUnder) {
            java.util.List<Node> swingChildren = ((GroupNode) parentNode).getSwingChildren();
            synchronized (parentNode) {
                java.util.List<Node> modelChildren = ((GroupNode) parentNode).getChildren();
        ...
        java.util.List<Map.Entry<Node, Collection<Node>>> changed;
        synchronized (changedNodesToFire) {
    ...
    private static UsageFilteringRule[] getActiveFilteringRules(final Project project) {
        ...
        java.util.List<UsageFilteringRule> list = new ArrayList<>(providers.length);
    ...
    private static UsageGroupingRule[] getActiveGroupingRules(@NotNull final Project project) {
        ...
        java.util.List<UsageGroupingRule> list = new ArrayList<>(providers.length);
    ...
    private void initTree() {
        ...
        DependencyDoubleClickHandler.install(myTree);
...
//                actionsManager.createExportToTextFileAction(myTextFileExporter),
    ...
    private AnAction[] createGroupingActions() {
        final UsageGroupingRuleProvider[] providers = Extensions.getExtensions(UsageGroupingRuleProvider.EP_NAME);
        java.util.List<AnAction> list = new ArrayList<>(providers.length);
        for (UsageGroupingRuleProvider provider : providers) {
    ...
    private void rulesChanged() {
        ...
        final java.util.List<UsageState> states = new ArrayList<>();
        if (myTree != null) {
            captureUsagesExpandState(new TreePath(myTree.getModel().getRoot()), states);
        }
        final java.util.List<Usage> allUsages = new ArrayList<>(myUsageNodes.keySet());
    ...
    private class MergeDupLines extends RuleAction {
        private MergeDupLines() {
            super(LibUsageView.this, UsageViewBundle.message("action.merge.same.line"), AllIcons.Toolbar.Filterdups);
    ...
    private Node[] getSelectedNodes() {
        ...
        final java.util.List<Node> result = new ArrayList<>();
    ...
    public java.util.List<Usage> getSortedUsages() {
        java.util.List<Usage> usages = new ArrayList<>(getUsages());
    ...
    private static Navigatable[] getNavigatablesForNodes(Node[] nodes) {
        ...
        final java.util.List<Navigatable> result = new ArrayList<>();
    ...
    private class MyPanel extends JPanel implements TypeSafeDataProvider, OccurenceNavigator, Disposable {
                ...
                public Collection<String> getTextLinesToCopy() {
                    ...
                    if (selectedNodes != null && selectedNodes.length > 0) {
                        ...
                        for (Node node : selectedNodes) {
                            lines.add(node.getText(LibUsageView.this));
                        }
                        return lines;
                    }
                    return null;
        ...
        public void calcData(final DataKey key, final DataSink sink) {
            if (key == CommonDataKeys.PROJECT) {
                ...
                sink.put(USAGE_VIEW_KEY, LibUsageView.this);
...
//                sink.put(PlatformDataKeys.EXPORTER_TO_TEXT_FILE, myTextFileExporter);
    ...
    private final class ButtonPanel extends JPanel {
        private ButtonPanel() {
            ...
            getProject().getMessageBus().connect(LibUsageView.this).subscribe(DumbService.DUMB_MODE, new DumbService.DumbModeListener() {
                @Override
