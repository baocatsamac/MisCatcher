...
public class SearchForUsagesRunnable implements Runnable {
    ...
    private final AtomicReference<LibUsageView> myUsageViewRef;
    ...
    private final LibUsageViewManager myUsageViewManager;
    private final AtomicInteger myOutOfScopeUsages = new AtomicInteger();

    SearchForUsagesRunnable(@NotNull LibUsageViewManager usageViewManager,
                            @NotNull Project project,
                            @NotNull AtomicReference<LibUsageView> usageViewRef,
    ...
    private static String createOptionsHtml(@NonNls UsageTarget[] searchFor) {
        KeyboardShortcut shortcut = LibUsageView.getShowUsagesWithSettingsShortcut(searchFor);
        String shortcutText = "";
        if (shortcut != null) {
    ...
    private static String presentableFileInfo(@NotNull VirtualFile vFile) {
        return getPresentablePath(vFile)
                + "&nbsp;("
                + LibUsageViewManager.presentableSize(LibUsageViewManager.getFileLength(vFile))
                + ")";
    }
    ...
    private static void flashUsageScriptaculously(@NotNull final Usage usage) {
        if (!(usage instanceof com.intellij.usages.UsageInfo2UsageAdapter)) {
            return;
        }
        com.intellij.usages.UsageInfo2UsageAdapter usageInfo = (UsageInfo2UsageAdapter)usage;
    ...
    private LibUsageView getUsageView(@NotNull ProgressIndicator indicator) {
        LibUsageView usageView = myUsageViewRef.get();
        ...
        if (usageCount >= 2 || usageCount == 1 && myProcessPresentation.isShowPanelIfOnlyOneUsage()) {
            usageView = new LibUsageView(myProject, myPresentation, mySearchFor, mySearcherFactory);
            ...
            if (myUsageViewRef.compareAndSet(null, usageView)) {
                if (myProcessPresentation.isShowFindOptionsPrompt()) {
                    ...
                    LibUsageView[] tmp = new LibUsageView[]{usageView};
    ...
    private void openView(@NotNull final LibUsageView usageView) {
        SwingUtilities.invokeLater(() -> {
            if (myProject.isDisposed()) return;
    ...
    private void searchUsages(@NotNull final AtomicBoolean findStartedBalloonShown) {
        ...
        findUsagesStartedBalloon.addRequest(() -> {
            notifyByFindBalloon(null, MessageType.WARNING,
                    Collections.singletonList(StringUtil.escapeXml(LibUsageViewManager.getProgressTitle(myPresentation))));
        ...
        usageSearcher.generate(usage -> {
            ...
            if (!LibUsageViewManager.isInScope(usage, mySearchScopeToWarnOfFallingOutOf)) {
            ...
            if (incrementCounter) {
                ...
                final LibUsageView usageView = getUsageView(indicator1);
                ...
                if (usageCount > UsageLimitUtil.USAGES_LIMIT && tooManyUsagesStatus.switchTooManyUsagesStatus()) {
                    LibUsageViewManager.showTooManyUsagesWarningLater(myProject, tooManyUsagesStatus, indicator1, myPresentation, usageCount, usageView);
                }
    ...
    private void endSearchForUsages(@NotNull final AtomicBoolean findStartedBalloonShown) {
        ...
        if (usageCount == 0 && myProcessPresentation.isShowNotFoundMessage()) {
            ApplicationManager.getApplication().invokeLater(new Runnable() {
                ...
                public void run() {
                    ...
                    if (myOutOfScopeUsages.get() != 0) {
                        lines.add(LibUsageViewManager.outOfScopeMessage(myOutOfScopeUsages.get(), mySearchScopeToWarnOfFallingOutOf));
                    }
                    if (myProcessPresentation.isShowFindOptionsPrompt()) {
            ...
            final LibUsageView usageView = myUsageViewRef.get();
            if (usageView != null) {
            ...
            if (myOutOfScopeUsages.get() == 0 || getPsiElement(mySearchFor)==null) {
                ...
                lines = Arrays.asList(LibUsageViewManager.outOfScopeMessage(myOutOfScopeUsages.get(), mySearchScopeToWarnOfFallingOutOf), createSearchInProjectHtml());
