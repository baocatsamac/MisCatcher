...
import ui.usages.PermissionDependenciesUsagePanel;
...
public class PermissionDependenciesPanel extends JPanel implements Disposable {
    ...
    private final PermissionDependenciesUsagePanel myUsagesPanel;
    ...
    private final PermissionDependenciesBuilder myBuilder;
    ...
    private final boolean isLayoutDependencyOnly;
    private final PsiFile layoutFile;
    private final String requestedPermissionGroup;

    public PermissionDependenciesPanel(Project project, PsiFile layoutFile, String requestedPermissionGroup, UIElement uiElement, Method method, boolean isLayoutDependencyOnly) {
        ...
        PermissionDependenciesBuilder dependenciesBuilder = PermissionDependenciesBuilder.getInstance(project);
        dependenciesBuilder.setDependenciesBuilderAttributes(uiElement, method, isLayoutDependencyOnly);
...
//        myDependencies.put(uiElement.getCodeFile(), null);
//        myDependencies.put(uiElement.getXmlFile(), null);
//        putAllDependencies(myBuilder);

//        exclude(excluded);
        this.myProject = project;
        this.myUsagesPanel = new PermissionDependenciesUsagePanel(this.myProject, this.myBuilder);
        ...
        myLeftTree.getSelectionModel().addTreeSelectionListener(e -> {
            ...
            if (searchIn.size() > 0){
                ArrayList<PsiElement> layoutPositions = retrieveMismatchLayoutPositions();
                if (isLayoutDependencyOnly){
                    // for the case of overuse or mismatch permission request --> show detailed mismatch in layout
                    myUsagesPanel.showUsages(null, (XmlFile) layoutFile, layoutPositions, null);
                } else {
                    // for the case of listener handler mismatch
                    myUsagesPanel.showUsages((PsiJavaFile) uiElement.getCodeFile(), (XmlFile) uiElement.getXmlFile(), layoutPositions, this.method.getCodeLinePermissionMappings());
                }
            }
        });
...
//        initTree(myRightTree, true);
    ...
    private ArrayList<PsiElement> retrieveMismatchLayoutPositions(){
        ArrayList<PsiElement> layoutPositions = new ArrayList<>();
        if (this.isLayoutDependencyOnly){
            for (Map.Entry<XmlTag,ArrayList<String>> map : Holder.getTagKeywordsMapping().entrySet()){
                if (map.getKey().getContainingFile().equals(this.layoutFile)){
                    ArrayList<String> inferredPermissions = PermissionHelper.inferPermissionsFromText(map.getValue());
                    if (inferredPermissions.size() != 0 && !inferredPermissions.contains(this.requestedPermissionGroup)){
                        layoutPositions.add(map.getKey());
                    }
                }
            }
        } else {
            for (Map.Entry<XmlTag,ArrayList<String>> map : Holder.getTagKeywordsMapping().entrySet()){
                if (map.getKey().getContainingFile().equals(this.uiElement.getXmlFile())){
                    ArrayList<String> inferredPermissions = PermissionHelper.inferPermissionsFromText(map.getValue());
                    if (inferredPermissions.size() != 0){
                        layoutPositions.add(map.getKey());
                    }
                }
            }
...
//        group.add(new MarkAsIllegalAction());
    ...
    private void initTree(final MyTree tree, boolean isRightTree) {
        tree.setCellRenderer(new MyTreeCellRenderer(this.uiElement, this.method));
    ...
    private void updateLeftTreeModel() {
        ...
        Set<PsiFile> analyzedFiles = new HashSet<>();
        if (!this.isLayoutDependencyOnly){
            analyzedFiles.add(uiElement.getCodeFile());
            analyzedFiles.add(uiElement.getXmlFile());
        } else if (layoutFile != null){
            analyzedFiles.add(layoutFile);
        }

        myLeftTreeExpansionMonitor.freeze();
        myLeftTree.setModel(buildTreeModel(analyzedFiles, myLeftTreeMarker));
    ...
    protected class MyTreeCellRenderer extends ColoredTreeCellRenderer {
        ...
        public MyTreeCellRenderer(UIElement uiElement, Method method){
            this.uiElement = uiElement;
            this.method = method;
        }
        @Override
...
//        myRightTree.getEmptyText().setText(emptyText);
    ...
    private final class EditDependencyRulesAction extends AnAction {
        ...
        @Override
        public void actionPerformed(AnActionEvent e) {
            boolean applied = ShowSettingsUtil.getInstance().editConfigurable(PermissionDependenciesPanel.this, new DependencyConfigurable(myProject));
            if (applied) {
                rebuild();
            }
        }
